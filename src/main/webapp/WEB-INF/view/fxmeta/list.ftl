                ...
                <div class="ibox-content">
                    <form class="form-inline row" id="J_formSearch" action="javascript:;">
                        ...
                        <div class="form-group col-md-2">
                            <button type="button" class="btn btn-w-m btn-info" onclick="MyFun.search('J_formSearch','FxMetaList');"><i class="fa fa-search"></i>查 询</button>
                        </div>
                    </form>
                    <div class="m-t">
                         <a data-toggle="modal" class="btn btn-primary"   data-target="#info-form" onclick="javascript:MyFun.appendAjaxHtml('${request.contextPath}/fxmeta/byid.do','info-form')">新增</a>
                    </div>
                    <div id="FxMetaList" data-url="${request.contextPath}/fxmeta/table.do"></div>    
                </div>
            </div>
        </div>
    </div>
...
function refreshFxMetaList(){
    MyFun.ajaxRefreshTable("FxMetaList");
}
...
function refresh () {
    ...
    G_tr.addClass("open");
    var _str = '<tr class="zChildTr"><td></td><td colspan="4"><div id="'+G_td_id+'"  data-url="'+"${request.contextPath}/fxmetadatas/table.do?metaid="+G_td_id+'"></div></td></tr>';
    G_tr.after(_str);
...
$(function(){
    MyFun.ajaxRefreshTable("FxMetaList");
    $('#FxMetaList').on('click', 'td.zzz', function () {
        var _this = $(this);
