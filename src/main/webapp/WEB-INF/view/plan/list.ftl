        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="m-t m-b">
                    ...
                    <a class="J_demoDownload" href="${request.contextPath}/plan.xls" target="_blank">示例下载</a>
                    <a class="J_demoDownload" href="javascript:;" id="J_demoImgShwo">示例图片查看</a>
                    <div id="J_demoImg" class="fn-hide" ><img src="${request.contextPath}/plan.png"></div>
                    <span class="ztips">（请按照示例表格格式上传数据！）</span>
                </div>
                <div id="PlanList" data-url="${request.contextPath}/plan/table.do">
                </div>    
               </div>
          </div>
            </div>
...
MyFun.ajaxRefreshTable=function(divid,url,callback){
      ...
      if(data.currentPage && !btnClickFlag){
            window.parent.setCurPageCookie('plan_currentPage', data.currentPage);
      }else{
          data.currentPage = window.parent.getCurPageCookie('plan_currentPage');
      }
      btnClickFlag = 0;
...
function zbtnSearch(){
    btnClickFlag = 1;
    window.parent.setCurPageCookie('plan_currentPage', '1');
    MyFun.search('J_formSearch','PlanList');
}

function refreshPlanList(){
   	MyFun.ajaxRefreshTable("PlanList");
}
...
function initExcelUpload(){
    ...
    var uploader = WebUploader.create({
        ...
        // 文件接收服务端。
        server: "${request.contextPath}/plan/uploadxls.do",
        // 选择文件的按钮。可选。
