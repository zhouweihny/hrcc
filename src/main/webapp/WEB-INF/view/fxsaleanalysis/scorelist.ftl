...
function getDetail(){
    createBlock(".wrapper", '正在努力加载数据...');
    $.PostJson("${request.contextPath}/fxsaleanalysis/scoretable.do", sortdataS+"&treeid="+curTreeId+"&storeid="+p_getStoreId+"&startdate="+startdate+"&enddate="+enddate, function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
                ...
                MyFun.to.e(json.message || "查询品种分析失败");
            }
        }
        clickFlag = 0;
        unBlock(".wrapper");
