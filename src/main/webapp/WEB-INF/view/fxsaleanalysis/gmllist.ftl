...
<tr>
    ...
    <td class="fn-text-right">{{retFixS gmlv}}</td>
</tr>
{{/each}}
</textarea>
...
function getDetail(){
    ...
    createBlock(".wrapper", '正在努力加载数据...');
    $.PostJson("${request.contextPath}/fxsaleanalysis/gmltable.do", "storeid="+p_getStoreId+"&startdate="+startdate+"&enddate="+enddate, function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
                ...
                MyFun.to.e(json.message || "查询千人购买率失败");
            }
        }
        unBlock(".wrapper");
    })
