...
<section class="fn-hide J_router_main">
    <header class="J_header">
        ...
        <h1 class="J_header-title">培训课件</h1>
        ...
        <table class="J_dataTbl" id="OrderList">
            <thead>
                <tr role="row">
                    <th>一级节点</th>
                    <th>二级节点</th>
                    <th class="fn-text-right">查看文章</th>
                </tr>
            </thead>
            <tbody id="J_tbl_body">
...
<tr>
    <td>{{lv1}}</td>
    <td>{{lv2}}</td>
    <td class="fn-text-right"><a href="javascript:;" class="J_showDetail" data-id="{{treeid}}" data-tit="{{name}}">查看</a></td>
</tr>
{{/each}}
</textarea>
...
function getDetail(){
    createLoading();
    $.PostJson("${request.contextPath}/fxorderfx/table2.do", "stype=desc&sfield=frequency&storeid="+p_getStoreId+"&startdate="+startdate+"&enddate="+enddate, function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
