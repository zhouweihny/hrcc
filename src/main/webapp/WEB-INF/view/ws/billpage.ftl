    ...
    $.AjaxHtml(srvMap.get("billpage_item"), "pageSize="+pageSize+"&currentPage="+currentPage, function(state, html){
        if(state == 'success'){
            ...
            $("#J_wxcontent").off("click").on("click", ".item", function(e){
                ...
                curBillStatus = _this.data("status");
                if(curBillStatus == '2' || curBillStatus == '3'){
                    billAvtime = _this.data("arrivaltime");
                }else{
                    billAvtime = null;
                }
                getSubPage();
...
function initSubPageBtns(){
    if(curBillStatus == '3'){
        $(".J_status_btn").off("click").find(".fa-caret-down").addClass("fn-hide");
    }else{
        $(".J_status_btn").find(".fa-caret-down").removeClass("fn-hide");
    ...
    $.PostJson(srvMap.get("arrivaltime"), "billid="+billid+"&arrivaltime="+cdate, function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
                ...
                curBillObj.data("arrivaltime", cdate);
                billAvtime = cdate;
            }else{
    ...
    $.PostJson(srvMap.get("save"), "id="+billid+"&status=3", function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
                ...
                curBillObj.data("status", "3");
                curBillObj.find(".weui-panel__hd .status").attr({"class": ""}).addClass("status s_3").text("已发货");
    ...
    $.PostJson(srvMap.get("save"), "id="+billid+"&status=2", function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
                ...
                curBillObj.data("status", "2");
                curBillObj.find(".weui-panel__hd .status").attr({"class": ""}).addClass("status s_2").text("已确认");
