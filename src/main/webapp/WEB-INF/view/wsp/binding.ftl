...
<header class="J_header">
    <h1 class="J_header-title">手机绑定</h1>
</header>
        ...
        <div class="weui-cells weui-cells_form">
            <div class="weui-cell">
                ...
                <div class="weui-cell__bd">
                    <input class="weui-input" value="" type="tel" placeholder="请输入手机号" id="mobileno" maxlength="11">
                </div>
            </div>
            <div class="weui-cell weui-cell_vcode">
                <div class="weui-cell__hd"><label class="weui-label">验证码</label></div>
                <div class="weui-cell__bd">
                    <input class="weui-input" type="text" placeholder="请输入验证码" maxlength="4" id="mcode">
                </div>
                <div class="weui-cell__ft">
                    <button class="weui-vcode-btn" id="J_code_btn">获取验证码</button>
                </div>
            </div>
        </div>

        <div class="zui-btn-wrap">
            <a href="javascript:;" class="weui-btn weui-btn_primary" id="J_bind_btn">立即绑定</a>
        </div>

        <a id="J_ret_count" class="J_returnBtn"><small>账号绑定</small></a>
    </div>
</div>
    ...
    <div class="weui-popup__modal">
        ...
        <div class="J_content">
            <div class="J_login_form" id="J_mobile_form">
                ...
                <a id="J_ret_mobile" class="J_returnBtn"><small>手机绑定</small></a>
            </div>
        </div>
    </div>
</div>
...
srvMap.add("sendcode", "success.json", "${request.contextPath}/wsp/sendcode.do");//获取验证码
srvMap.add("bindClerk", "success.json", "${request.contextPath}/wsp/verifycode.do");//绑定
srvMap.add("banduser", "success.json", "${request.contextPath}/wsp/banduser.do");//绑定
...
$(function(){
    ...
    if(bind_username){
        ...
        $("#J_ret_count").click(function(){
            $("#J_zuisubPage").popup();
        })
        $("#J_closeZuiSubPage, #J_ret_mobile").click(function(){
            $.closePopup();
        })
...
function getCode(){
    ...
    createLoading();
    countNum();
    $.PostJson(srvMap.get("sendcode"), "mobileno="+mobileno, function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
                zuiToptip("短信验证码已发送到您的手机，请注意查收");
            }else{
                zuiAlert(json.message||"获取验证码失败");
            }
            clickFlag = 0;
        }else{
            zuiAlert("网络错误，请稍后重试");
...
function countNum(){
    $("#J_code_btn").text(counter+'"后重试');
    counterInter = setInterval(function(){
        if(counter == 1){
            stopCount();
        }else{
            counter--;
...
function bindClerk(){
    ...
    clickFlag = 1;
    createLoading();
    $.PostJson(srvMap.get("bindClerk"), "mobileno="+mobileno+"&mcode="+mcode, function(state, json){
        if(state == 'success'){
            if(json && json.code == '0000'){
                zuiTotast("绑定手机号成功", "", function(){
                    console.log("跳转")
