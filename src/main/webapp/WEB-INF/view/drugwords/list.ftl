                ...
                <div class="ibox float-e-margins">
                    ...
                    <form class="form-inline row" id="J_formSearch" action="javascript:;">
                        <div class="form-group col-md-3">
                            <label for="J_name">同义词：</label>
                            <input type="text" class="form-control" id="J_name" placeholder="输入同义词" name="name">
                        </div>
                        <div class="form-group col-md-3">
                        ...
                        <div class="form-group col-md-2">
                            <button type="button" class="btn btn-w-m btn-info" onclick="MyFun.search('J_formSearch','DrugwordsList');"><i class="fa fa-search"></i>查 询</button>
                        </div>
                    </form>
                    <div class="m-t">
                         <a data-toggle="modal" class="btn btn-primary"   data-target="#info-form" onclick="javascript:MyFun.appendAjaxHtml('${request.contextPath}/drugwords/byid.do','info-form')">新增</a>
                   		 <a class="btn btn-primary" id="J_excelUpload">上传</a>
                    </div>
                    <div id="DrugwordsList" data-url="${request.contextPath}/drugwords/table.do">
                    </div>    
                   </div>
              </div>
                </div>
...
var clickFlag = 0;
  ...
  function refreshDrugwordsList(){
   	MyFun.ajaxRefreshTable("DrugwordsList");
  }

  $(function(){
     refreshDrugwordsList();
     initExcelUpload();
  })
  ...
  function initExcelUpload(){
    ...
    var uploader = WebUploader.create({
        ...
        // 文件接收服务端。
        server: "${request.contextPath}/drugwords/uploadxls.do",
        // 选择文件的按钮。可选。
