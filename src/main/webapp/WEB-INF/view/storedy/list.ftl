                ...
                <div class="ibox-content">
                    <form class="form-inline row m-b" id="J_formSearch" action="javascript:;">
                        <div class="form-group col-md-3">
                            <label for="zuiStoreId">店员名：</label>
                    ...
                    <div id="StoreDyList" data-url="${request.contextPath}/storedy/table.do"></div>
                </div>
            </div>
        </div>
    </div>
...
<div id="info-form" class="modal fade" aria-hidden="true" >
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>店员管理<span class="J_quick_dura"></span></h5>
...
<div class="form-group">
    <label class="col-sm-3 control-label">店员名：</label>
    <div class="col-sm-8">
        <input type="text" class="form-control" placeholder="输入店员名" name="dyname">
    </div>
</div>
<div class="form-group">
...
function refreshDrugList(){
    MyFun.ajaxRefreshTable("StoreDyList");
}
...
function getDetail(){
    MyFun.search('J_formSearch','StoreDyList');
}
...
function saveForm(){
    ...
    var str = "id="+cid+"&storeid="+storeid+"&name="+dyname+"&phone="+phone+"&remark="+remark+"&status="+state;

    createBlock(".modal-dialog", '正在努力加载数据...');
    $.PostJson("${request.contextPath}/storedy/save.do", str, function(state, json){
        if(state == 'success'){
