...
var refresher = {
	...
	init: function(parameter) {
		...
		var list = wrapper.querySelector("#" + parameter.id + " " + (parameter.item || "li"));
	...
	scrollIt: function(parameter, pullDownEl, pullDownOffset, pullUpEl, pullUpOffset) {
		...
		document.addEventListener('touchmove', function(e) {
			/**
			 * modby zw 2018年2月9日09:27:24
			 * 当iscroll页面上有popup等其他需要滚动的组件时，
			 * 不能阻断，否则无法滚动
			 * subPopPage: true
			**/
			if(!parameter.subPopPage)
				e.preventDefault();
		}, false);
	},
	onScrolling: function(e, pullDownEl, pullUpEl, pullUpOffset) {
		...
		//回退不上拉
		if ((e.scrollerH < e.wrapperH && e.y > (e.minScrollY - pullUpOffset) && pullUpEl.id.match('flip') || e.scrollerH > e.wrapperH && e.y > (e.maxScrollY - pullUpOffset) && pullUpEl.id.match('flip')) == null) {
			pullUpEl.querySelector('.pullUpLabel').innerHTML = refresher.info.pullUpLable;
		}
	},
	...
	onPulling: function(pullDownEl, pullDownAction, pullUpEl, pullUpAction) {
		if (pullDownEl.className.match('flip') && (pullDownEl.querySelector('.pullDownLabel').innerHTML == refresher.info.pullingDownLable) ) {
		...
		if (pullUpEl.className.match('flip') && (pullUpEl.querySelector('.pullUpLabel').innerHTML == refresher.info.pullingUpLable)) {
