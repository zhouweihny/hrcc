 ...
 * {{/if_range}}
 */
Handlebars.registerHelper('if_range', function(context, list, options) {
	var listArr = list.split(",");
	if(listArr.length == 2){
		if(context > listArr[0] && context < listArr[1]){
			return options.fn(this);
		}else{
			return options.inverse(this);
	    }
	}else{
		throw new Error("Handlebars 自定义 if_range 方法传入参数错误！"); 
	}
});
