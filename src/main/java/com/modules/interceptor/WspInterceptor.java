 ...
 */
@Repository
public class WspInterceptor extends HandlerInterceptorAdapter {
	...
	@Value("#{configProperties['wsp.code']}")
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (request.getSession().getAttribute(Constants.WX) == null) {
			...
			if (!StringUtils.isEmpty(code)) {
				...
				if (code.equals("test")) {
					sysWxUser.setOpenid("oQslywaL5XwH8UIgxKz4rocBtYWA");
				} else {
				...
				if (StringUtils.isEmpty(sysWxUser.getUserid())) {
					response.sendRedirect(path + "/wsp/binding.do?redirect=" + URLEncoder.encode(request.getRequestURI() + "?" + request.getQueryString(), "utf-8"));
					return false;
				}
				User user = new User();
